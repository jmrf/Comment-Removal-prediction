...
def convert_padding_direction(src_tokens, padding_idx,
                              right_to_left=False, left_to_right=False):
...
class SentenceEncoder:
    ...
    def __init__(self, model_path, max_sentences=None, max_tokens=None,
                 cpu=False, fp16=False, sort_kind='quicksort'):
    ...
    def _make_batches(self, lines):
        ...
        def batch(tokens, lengths, indices):
            toks = tokens[0].new_full(
                (len(tokens), tokens[0].shape[0]), self.pad_index)
        ...
        for i in indices:
            if nsentences > 0 and (
                (self.max_tokens is not None and
                    ntokens + lengths[i] > self.max_tokens) or
                (self.max_sentences is not None and
                    nsentences == self.max_sentences)):
...
class Encoder(nn.Module):
    def __init__(self, num_embeddings, padding_idx, embed_dim=320,
...
def EncodeFile(encoder, inp_fname, out_fname, buffer_size=10000,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--token-lang', type=str, default='--',
                        help="Perform tokenization with given language"
                             " ('--' for no tokenization)")
